...
def handle_argv
  ...
  opt.on('-r', '--[no-]reverse', sprintf("Reverse the behaviour (print BEFORE NUM-th line - inclusive|exclusive) (Def: %s)", (!OPTS[:inverse]).inspect), TrueClass) {|v| OPTS[:inverse] = v}  # WARNING-NOTE: the Hash keyword is "inverse" as opposed to "reverse"
  ...
  opt.separator "Note:"
  opt.separator "  UNIX command of 'tail -n +5' is equivalent to 'head.rb -r -n 5'"
